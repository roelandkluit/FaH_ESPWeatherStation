    ...
    for (int i = 0; i < _paramsCount; i++) {
     ...
     String pitem;
      ...
      if (_params[i]->getID() != NULL) {
        ...
        {
            if (tok_v)pitem.replace(FPSTR(T_v), PWD_MASK); // T_v value
...
void WiFiManager::doParamSave(){
  ...
  if(_paramsCount > 0){
    ...
    for (int i = 0; i < _paramsCount; i++) {
      ...
      if ((strlen(_params[i]->getCustomHTML()) > 0) && (strstr(_params[i]->getCustomHTML(), "password") != NULL) && (value.compareTo(PWD_MASK) == 0))
      {
        #ifdef WM_DEBUG_LEVEL
        DEBUG_WM(DEBUG_VERBOSE, "KeepPwd:", _params[i]->_value);
        #endif
      }
      else
      {
